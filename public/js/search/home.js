        ...
        let Search_Result_Page = {
            ...
            _eventsSearch: function () {
                ...
                $('input[name="checkInOut"]', base.search_container).on('daterangepicker_change', function (e, start, end) {
                    ...
                    if (base.dataFilter['checkIn'] !== checkIn || base.dataFilter['checkOut'] !== checkOut && checkIn !== checkOut) {
                ...
                $('input[name="checkInOutTime"]', base.search_container).on('daterangepicker_change', function (e, start, end) {
                    let checkIn = start.format('YYYY-MM-DD'),
                        checkOut = end.format('YYYY-MM-DD'),
                        checkInOut = checkIn + '+12:00+am-' + checkOut + '+11:59+pm';
                    if (typeof checkIn == 'string' && typeof checkOut == 'string') {
                        base.dataFilter['checkInTime'] = checkIn;
                        base.dataFilter['checkOutTime'] = checkOut;
                        base.dataFilter['checkInOutTime'] = checkInOut;
                        base._resetPage();
                        base._searchCallBack();
                        base.pushStateToFilter('checkInTime', checkIn);
                        base.pushStateToFilter('checkOutTime', checkOut);
                        base.pushStateToFilter('checkInOutTime', checkInOut);
                    }
                });
                ...
                $('#hh-map-toggle-search', base.search_container).on('change', function () {
                    ...
                    if ($(this).is(':checked')) {
                        ...
                        base.dataFilter['number'] = 10;
                        base.pushStateToFilter('number', 10);
                                                    ...
                                                    if ($('.hh-map-price[data-id="' + item.post_id + '"]', base.search_container).length === 0) {
                                                        let popup = new mapboxgl.Popup({
                                                            ...
                                                            .setHTML('<div class="hh-map-popup">' +
                                                                '<div class="thumb"><a href="' + item.url + '"><img src="' + item.thumbnail + '" alt="' + item.title + '"/></a></div><div class="content"><a href="' + item.url + '">' + item.title + '</a><p class="pr">' + item.price + '</p></div>' +
