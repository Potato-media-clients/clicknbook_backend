...

{
	...
	public function get_home_by_type(Request $request){
        ...
        $find=DB::select("select * from wishlist WHERE user_id='$user_id' AND post_id='$id' AND status='1'");

        foreach($find as $finds){$status=(bool)$finds->status;}}
	...
	public function DistinationsHome(Request $request){
        ...
        foreach($find as $finds){$status=(bool)$finds->status;}}
			...
			'results' => 'Distination Is Required'
	...
	public function customerHomes(Request $request){
		...
		$posts = $this->model->getSearchResult1($data);
		...
		$id=$v->author;
               ...
               if($id==$user_id){
				...
				$temp->post_content = get_attachment_url($v->post_content);
    ...
    {   
        ...
        if($data){
            ...
            $data->post_content =get_attachment_url($data->post_content);
