...

{
...

    {
        ...
        $enquiry_options = get_option('send_enquire_email', 'admin_partner_customer');

        $enquiry_options = explode('_', $enquiry_options);



        $email_from = get_option('email_from_address', $email);

        $email_name = get_translate(get_option('email_from', $name));



        if(in_array('admin', $enquiry_options)){

            $admin = get_admin_user();

            send_mail(esc_html($email_from), esc_html($email_name), $admin->email, sprintf(__('[%s] Have a booking request from [Home ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));

        }

        if(in_array('partner', $enquiry_options)){

            $partner = get_user_by_id($post->author);

            send_mail(esc_html($email_from), esc_html($email_name), $partner->email, sprintf(__('[%s] Have a booking request from [Home ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));

        }

        if(in_array('customer', $enquiry_options)){

            send_mail(esc_html($email_from), esc_html($email_name), $email, sprintf(__('[%s] You have created a booking request for [Home ID: %s] - %s'), $site_name, $post_id, $post_title), balanceTags($message));
    ...
    public function listOfHomes1($data = [])

    {

        $home = new Home();

        return $home->listOfHomes1($data);
...

    {
        ...
        $number_infants = (int)request()->get('num_infants');
        ...
        if (hh_compare_encrypt($homeID, $homeEncrypt) && $startDate && $endDate) {
            ...
            $price_neight = Home::find($homeID);
            $price_neight = $price_neight->base_price;
            $this->sendJson([

                'status' => 1,

                'html' => view('frontend.home.calculate-price-render',
                 [
                 'startTime'=> request()->get('checkIn'), 
                 'endTime'=> request()->get('checkOut'),
                 'number_adults' => $number_adults,
                 'price_neight'=>$price_neight,
