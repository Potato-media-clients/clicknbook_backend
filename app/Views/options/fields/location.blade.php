...
<div id="setting-{{ $idName }}" data-condition="{{ $condition }}"
    ...
    <div class="col col-sm-6">
        ...
        <div class="form-group">
            ...
            @foreach($langs as $key => $item)   
            <select  class="form-control hh-country {{get_lang_class($key, $item)}}"  value="{{ get_translate($value['country'], $item) }}" name="{{ $id }}[country]{{!empty($item) ? '['. $item .']' : ''}}" id="{{ $idName }}_country{{get_lang_suffix($item)}}">
