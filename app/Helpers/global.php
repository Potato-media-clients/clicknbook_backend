...
function list_tabs_services_sorted(){
    $enable_services = convert_tab_service_to_list_item();
    $tab_services = get_option('sort_search_form', $enable_services);
    $tab_services_convert = [];
    if(!empty($tab_services)){
        foreach ($tab_services as $item){
            $tab_services_convert[$item['id']] = $item;
        }
    }
    $services = array_merge($tab_services_convert, $enable_services);
    $return = [];
    if(!empty($services)){
        foreach ($services as $item){
            $enable = get_option('enable_' . $item['id'], 'on');
            if($enable == 'on'){
                $temp = $item;
                if(isset($tab_services_convert[$item['id']])){
                    $temp['label'] = $tab_services_convert[$item['id']]['label'];
                }
                $return[] = $temp;
            }
        }
